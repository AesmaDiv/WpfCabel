...
{
    public class xAdam5000TCP : IDisposable
    {
        private xClientAsync _client;                  // TCP клиент для общения с адамом
        ...
        {
            get { return _read_delay; }
            set { _read_delay = value; }
        }
        ...
        public void Disconnect()
        {
            ...
            {
                // Прекращаю чтение
                _tmr_read.Change(Timeout.Infinite, Timeout.Infinite);
                _reading = false;                
                // Собственно..
                _client.Disconnect();
            }
        ...
        public void SetReadingState(bool state)
        {
            ...
            _tmr_read.Change(0, _read_delay);
        }
        // *******************************************************************************************************************************
        #endregion
        ...
        private void _client_xEvent(object sender, EventArgs e)
        {
            try
            {
                ...
                xClientAsync.ClientEventArgs client_args = e as xClientAsync.ClientEventArgs;
